...
public class TileMapDestroyScript : MonoBehaviour {
	...
	IEnumerator setNWESTiles(int xPos, int yPos){
		...
		int _N = tilemap.Layers[3].GetTile(xPos,yPos+1);
		int _E = tilemap.Layers[3].GetTile(xPos+1,yPos);
		int _W = tilemap.Layers[3].GetTile(xPos-1,yPos);
		int _S = tilemap.Layers[3].GetTile(xPos,yPos-1);


		//get diagonal Tile ID's 
		int NW = tilemap.Layers[5].GetTile(xPos-1,yPos+1)-16;
		int NE = tilemap.Layers[5].GetTile(xPos+1,yPos+1)-16;
		int SE = tilemap.Layers[5].GetTile(xPos+1,yPos-1)-16;
		int SW = tilemap.Layers[5].GetTile(xPos-1,yPos-1)-16;

		//Debug.Log ("NW: " + NW + "NE: " + NE + "SE: " + SE + "SW: " + SE);
		//Get Tile ID's on layer 3 to check if tile is present, so cleared tiles can be skipped
		int _NW = tilemap.Layers[3].GetTile(xPos-1,yPos+1);
		int _NE = tilemap.Layers[3].GetTile(xPos+1,yPos+1);
		int _SE = tilemap.Layers[3].GetTile(xPos+1,yPos-1);
		int _SW = tilemap.Layers[3].GetTile(xPos-1,yPos-1);
		...
		if (_NW >= 0 ){
			...
			tilemap.Layers[5].SetTile(xPos-1,yPos+1, (convertAddBinary (NW,8) + 16 ));
		}
		if (_NE >= 0 ){
			if (NE <0){
				NE = 0;
			}
			tilemap.Layers[5].SetTile(xPos+1,yPos+1, (convertAddBinary (NE,4) + 16 ));
		}
		if (_SE >= 0 ){
			if (SE <0){
				SE = 0;
			}
			tilemap.Layers[5].SetTile(xPos+1,yPos-1, (convertAddBinary (SE,1) + 16 ));
		}
		if (_SW >= 0 ){
			if (SW <0){
				SW = 0;
			}
			tilemap.Layers[5].SetTile(xPos-1,yPos-1, (convertAddBinary (SW,2) + 16 ));
		}
